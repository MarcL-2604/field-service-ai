...
- Mail-Erzeugung getrennt von Mail-Versand: `generiereTechnikerMail()` / `generiereKundenMail()` geben `{empfaenger, betreff, text}` zurück (`generiereKundenMail()` zusätzlich `{von, cc}`). Der `mailto:`-Link (Button "In Outlook öffnen") ist nur EIN Consumer dieser Funktionen — nach der Pilotphase kommt SMTP/Graph-API als zweiter Consumer hinzu, ohne dass sich die Erzeugungslogik ändert.
- Kundenmail: Absender ist der zuständige Techniker (nicht der Koordinator), Koordinator steht in CC. Begründung: Rückfragen/Terminabsagen/Umplanung durch den Kunden landen so direkt beim Techniker; der Koordinator bleibt informiert und kann bei Bedarf eingreifen. Techniker-Benachrichtigung (Schritt 4) bleibt Koordinator → Techniker, kein CC.
- Schritt 7 (Abschluss): Techniker meldet aktiv ab (Formular: Geräte erledigt + Auffälligkeit je Gerät, Gesamtdauer, Dokumenten-Checkliste). `prüfeAbschluss(geplanteDauerMin, gemeldeteDauerMin, anzahlErledigt, anzahlGesamt, auffaelligkeiten, dokumente)` ist eine reine Funktion (kein UI-Code vermischt) und prüft 4 Faktoren — Vollständigkeit Geräte, Plausibilität Dauer (±20%/±40%-Toleranzstufen), Auffälligkeiten, Vollständigkeit Dokumente (Foto nur Pflicht bei gemeldeter Auffälligkeit). Schlechtester Einzelwert bestimmt die Gesamt-Ampel — später 1:1 gegen SMax-Rückmeldedaten austauschbar.
